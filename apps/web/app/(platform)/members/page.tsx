...
import type { PaginatedResponse, MemberListItem } from '@/types/api';
...
async function fetchMembersServer(
  ...
  const params = new URLSearchParams({ limit: '20', page: '1', ...filters });
  try {
...
export default async function MembersPage({ searchParams }: PageProps) {
  ...
  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <MemberDirectory initialFilters={filters} isAuthenticated={!!user} />
    </HydrationBoundary>
  );
}
